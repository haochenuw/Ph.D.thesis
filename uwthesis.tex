...
For every curve $X$ there is a nonnegative integer $g(X)$ called its {\em genus}, and a field $k(X)$ of transcendence degree 1 over $\bQ$, called the {\em function field} of $X$. We have the following
\begin{Fact}
\label{fact: degree}
If $x$ is a non constant rational function,  then $[k(X): k(x)] = \deg x$.
\end{Fact}

Let $k$ be a field. An {\em elliptic curve} defined over $k$ is the projective closure in $\bP^2$ of an affine curve given by the Weierstrass equation $y^2 = x^3 + Ax+B$ with $A,B \in k$ and $4A^3+27B^2 \neq 0$. Alternatively, an elliptic curve over $k$ is a nonsingular projective over $k$ of genus one with a distinguished rational point. 

Let $E$ be an elliptic curve defined over a field $k$.  It turns out that $E$ also has the structure of an algebraic group. 
When $k = \bR$, the group addition can be visualized as a ``secant-chord'' operation: suppose $P,Q \in E(\bR)$, and let 
$R$ be the third point of intersection of the line $PQ$ with $E$. Then $P+Q  = -R$. 

Therefore, an elliptic curve over $k$ is an abelian variety over $k$ of dimension 1. In particular, for any extension 
$k'/k$, the set $E(k')$ is an abelian group. 


As for the structure of $E(k)$, we recall the famous Mordell-Weil theorem:
\begin{theorem}
...
The integer $r$ in the above theorem is called the {\em rank} of $E(K)$. 
...
It turns out that $X_0(N)$ is a nonsingular projective algebraic curve. The genus of $X_0(N)$ is given by 
the following genus formula (see, for example, Silverman). 
\[
	g(X_0(N)) =  fixme
\]
...
Notation: Let $\H = \{z \in \bC: \Im(z) > 0\}$ denote the complex upper half plane.
...
\]
We will omit the $k$ in the subscript, since we always mean $k = 2$ when $f$ is a modular form of weight 2, and $k = 0$ when $f$ is a modular function.
\begin{Definition}
...
\end{Definition}
A modular form $f$ is a {\em cusp form} if $f$ vanishes at all cusps. For the precise definition of vanishing at cusps, see  \cite[I.2]{diamond2005first}.
