...
\end{equation}
Here $g(\chi)$ is the usual Gauss sum of $\chi$: if $\chi$ is a character modulo $d$, then $g(\chi) = \sum_{a=1}^{d} e^{\frac{2\pi i a}{d}} \chi(a)$.
\end{Definition}
...
\red{to-do: add formula for twists}
...
of $Gal(L/\bQ)$, where $L$ is the smalles t number field over which $E$ acquires good reduction \red{(to-do: check this)}.
