...
The order of vanishing of $L(E,s)$ at $s = 1$ is called the {\it analytic rank} of $E$.  The rank part of the famous Birch and Swinnerton-Dyer (BSD) conjecture states that the analytic rank is equal to the algebraic rank, i.e., 
\[
	\rank(E(\bQ)) = \ord_{s=1}L(E,s).
\]
When the analytic rank of $E$ is zero or one, the rank part of the BSD conjecture is proved by the important and beautiful work of Gross-Zagier and Kolyvagin. As of now this conjecture is still open for $\ord_{s=1}L(E,s) > 1$.
...
Another way to obtain rational points on elliptic curves is the construction of Chow-Heegner points.  It occured to me during my study of critical subgroups that the ideas developed for critical subgroups can also be applied to Chow-Heegner points, this motivated me to give an algebraic algorithm for their computation. This parallels the numerical algorithms of Darmon et al. in 
...
Recall that when the analytic rank of $E$ is one, we can construct points of infinite order on $E(\bQ)$ using {\it Heegner points}. However, when $r_{\an}(E) \geq 2$, it turns out that the images of Heegner points are always torsion. The critical subgroup is interesting, especially when $r_{\an}(E) \geq 2$, because it could potentially provide a way to construct rational points on elliptic curves. Hence we can ask the following question.
...
\end{problem}
Solving Problem~\ref{problem: expansion} is essentially equivalent to computing the map
\[
...
One motivation of studying the expansions at all cusps  is that they are useful  in evaluating modular functions on $X_0(N)$ near the cusps, which is an important part of the algorithms of  \cite{edixhoven2006computational} to compute the residual Galois representations $\bar{\rho}_{f,\lambda}$ associated to the newform $f$.   \\
...
In Chapter 5, we study {\it Chow-Heegner points}. Let $E,F$ be non-isogenous elliptic curves defined over $\bQ$ of the same conductor $N$. The Chow-Heegner point $P_{E,F} \in E(\bQ)$ is constructed by the following procedure: take any point on $F(\bC)$, take its inverse image on $X_0(N)$, then map that image down to $E$ and take the sum the resulting points (for a precise definition, see \cite{stein2012numerical}). In \cite{darmon2015algorithms}, Darmon, Daub, Lichtenstein and Rotger developed an algorithm to compute Chow-Heegner points via iterated integrals. In \cite{stein2012numerical}, Stein developed a fast and conceptually easy algorithm to numerically compute Chow-Heegner points. Yuan-Zhang-Zhang proved the the following theorem, in \cite{yuan2011triple}, which relates the height of Chow-Heegner point with certain triple-product $L$-function $L(E,F,F,s)$. 
...
Also, combining the method developed in Chapter 3 with the PARI function {\bf ell.taniyama}, I developed an algebraic algorithm to compute lifting of points of any modular parametrisation $X_0(N) \to E$.  Using this, I will give an algebraic algorithm to {\it provably} compute the Chow-Heegner point $P_{E,F}$.
...
\begin{project}
Prove that $\rank(E_{\crit}(\bQ)) = 0$ whenever $r_{\an}(E)$ is even, or give a counter example. 
\end{project} 
...
\begin{project}
Develop a numerical algorithm for computing local components $\pi_{f,p}$, with explicit complexity bounds.
\end{project}
...
\end{project}
\begin{project}
Verify the table of Chow-Heegner points in \cite{stein2012numerical} and extend the table.
\end{project}
