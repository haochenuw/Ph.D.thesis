...
Let $\bar{k}$ be a fixed algebraic closure of $k$. A {\em divisor} on a curve $X$ is a formal sum $D = \sum_{z \in X(\bar{k})} n_z[z]$ where $n_z = 0$ for all but finitely many $z \in X$. The set of divisors on $X$ form a free abelian group, which we denote by $\Div(X)$. 
...
Section~\ref{sec: examples} contains some examples. % In section, we describe an alternative approach and 
