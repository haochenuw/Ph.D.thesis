...
\[
	E: y^2 = x^{3} - 7 x + 10
\]
labeled as ``664a1'' in Cremona's table. Then $r_{\an}(E) = 2$, and $X_0(664)$ has genus 81.  Let $r = r_4$ be as defined in Remark~\ref{rem: in practice}. Using the method described in Remark~\ref{rem: expliciteta}, we found two $\eta$-products \\ 
...
\[
	E: y^2 + y = x^{3} + x^{2} - 2 x
\]
with rank 2 and Cremona label ``389a'' is presented in \cite[Appendix B.1]{delaunay2002thesis}. The results suggested that $\Div(\omega)$ contains two Heegner points of discriminant 19, and the critical subgroup $E_{\crit}(\bQ)$ is torsion. Using the critical $j$-polynomial for ``389a'' in Table~\ref{table: rank two}, we can confirm the numerical results of Delaunay.
\end{Remark}
...
Then 
\[
	\tau_s \left([\frac{a}{y}]\right) = \left[\frac{a}{ys'} \right], 
\]
...
\begin{proof}
From part (2) of Lemma~\ref{stevens}, we see that if $z$ is a cusp of denominator $d$ and $s \equiv1 \mod{d'}$, then $\tau_s(z) = z$.  The claims now follow directly from this observation.
\end{proof}

\begin{Prop} \label{field of definition}
We have \\
(1)	$\bQ(\{a_n(f, c)\}) \subseteq \bQ( \{a_n(f)\}, \zeta_{d'}).$ \\
(2) Let $\sigma \in Gal(\bar{\bQ}/\bQ)$  be such that $\sigma|_{\bQ(\zeta_{N})} = \tau_s$. Then 
\[
	(f_z)^{\sigma} = (f^{\sigma})_{\tau_s(z)}.
\]
\end{Prop}

\begin{proof}
First, (1) follows from (2), for if $\sigma$ fixes $\bQ( \{a_n(f)\}, \zeta_{d'})$, then $f^{\sigma} = \sigma$ and $\tau_s(z) = z$. To prove (2), first note that the function field of $X_0(N)$ is generated by j and $j_N$. So we may write write $f =  P(j,j_N)/Q(j,j_N)(dz)^k$, where $P, Q \in K_f[x,y]$. Now it suffices to prove it for $j_N$. \red{fixme: complete the proof}.
\end{proof}
...
Now by a formula in [Brunault, private communication], we have  $$\sum_{\chi: \cond(\chi) = p}  g(\bar{\chi})\epsilon_p(\pi_{f \otimes \chi}, 1/2, \psi) = \frac{-1}{p}\sum_{x \in \bF_{p^2}^{\times}} \psi(x+x^p+x^{p+1}) \varphi(x).$$
This combined with our Lemma~\ref{cuspidal constant} gives the result. 

Brunault, private communication.  \red{fixme: add ref}.
\end{proof}
