...
elliptic curves defined over $\mathbb{Q}$. Chapter 2 covers the background materials. In Chapter 3,
we develop algorithms to compute the Mazur Swinnerton-Dyer critical subgroup of elliptic curves, and verify that for all elliptic curves of rank two and conductor less than a thousand, the critical subgroup is torsion. 
In Chapter 4, we develop algorithms to compute Fourier expansions of $\Gamma_0(N)$-newforms at cusps other than the cusp at infinity. In Chapter 5 we study properties of Chow-Heegner points. We proved that the index of Chow-Heegner points are always divisible by two when the conductor $N$ has many prime divisors, and we develop algebraic algorithms to compute the Chow-Heegner points. 
}
...
Elliptic curves and modular forms are among the central themes of 20th century number theory. Since the celebrated Mordell-Weil theorem, there has been a great deal of research on ranks of elliptic curves. We highlight the great work of many people towards the Birch and Swinnerton-Dyer conjecture and the recent work of Bhargava et al. on average ranks of elliptic curves in families.  

The theory of modular forms is a special case of the theory of automorphic forms. Although the theory of modular forms belong to complex analysis, its importance mainly lie in its connections with number theory. Perhaps the most famous application of modular forms is the proof of Fermat's last theorem in the 1990's.  The proof reveals a deep connection between elliptic curves over $\bQ$ and certain modular forms of weight 2, called {\it newforms}. More precisely, the {\it modularity theorem} states that for every elliptic curve $E$ over $\bQ$ of conductor $N$, there is a newform $f$ of 
weight 2 and level $N$ such that their $L$-functions are equal, i.e., 
\[
	L(E,s) = L(f,s).
\]
Given an elliptic curve $E$ defined over $\bQ$, the modularity theorem can be restated in the form of existence of a surjective morphism
\[
	\varphi: X_0(N) \to E
\] 
from the modular curve $X_0(N)$. The map $\varphi$ is called a {\it modular parametrization}. The study of computational problems related to $\varphi$ will be the theme of this thesis. 

Motivated by studying the rank part of the Birch and Swinnerton-Dyer conjecture, I started researching the critical subgroups of elliptic curves, defined by Mazur and Swinnerton-Dyer in the 1970's. I developed algorithms to compute 
the critical subgroups and proved through computation that for all elliptic curves of rank 2 and conductor smaller than a thousand, the critical subgroup is trivial. 
...
In trying to compute critical subgroups, I was led to the question of computing the Fourier expansions of modular forms at all cusps, which turns out to be an interesting question in its own right, and has connections to the study of the automorphic representation attached to $E$. I develop an algorithm to compute such Fourier expansions in Chapter 4.
 ...
 Another way to obtain rational points on elliptic curves is the construction of Chow-Heegner points.  It also occurs to me during my study of critical subgroups that the ideas developed for critical subgroups can also be applied to Chow-Heegner points, this motivated me to give an algebraic algorithm for their computation. This parallels the numerical algorithm of Darmon et al. in 
\cite{darmon2015algorithms}. 

\section{Contributions}
In this thesis, we study certain computational problems arising from studying rational points on elliptic curves. 


\subsection{Chapter 3} We study the {\it critical subgroups} associated with elliptic curves over the rationals. Let $E$ be an elliptic curve defined over $\bQ$. Since $E$ is modular, there is a surjective morphism $\varphi:X_0(N) \to E$ over $\bQ$. Let $R_\varphi$ denote the ramification divisor of $\varphi$. Mazur and Swinnerton-Dyer defined the {\em critical subgroup} of $E$ in \cite{mazur-sd} as
\[
	E_{\crit}(\bQ)  = \langle \tr(\varphi([z])) : [z] \in \supp R_\varphi \rangle,
\]
where $\tr(P) = \sum_{\sigma: \bQ(P) \to \bar{\bQ}} P^{\sigma}$. \\

Recall that when the analytic rank of $E$ is less than or equal to one, we can construct points of infinite order on $E(\bQ)$ using {\it Heegner points}. However, when $r_{\an}(E) \geq 2$, it turns out that the images of Heegner points are always torsion. The critical subgroup is interesting, especially when $r_{\an}(E) \geq 2$, because it provides a way to construct rational points on elliptic curves. Hence we can ask the following question.

%Observe that $\deg R_{\varphi} = 2g(X_0(N))-2$. In addition, we will assume $E$ is an optimal elliptic curve, so $\varphi$ is unique up to sign.
...
\end{question}
In \cite{delaunay2002thesis}, Christophe Delaunay performed numerical computations for the critical subgroups, but $E_{\crit}(\bQ)$ has not been provably computed for a single curve $E/\bQ$. 
%In \cite{chen2014computing}, I proved the following theorem.
...
For all elliptic curves $E/\bQ$ of analytic rank two and conductor smaller than 1000, the rank of $E_{\crit}(\bQ)$ is zero.
\end{theorem}


\subsection{Chapter 4} 

In Chapter 4, I study the problem of Fourier expansion of newforms at all cusps. Let $k$ and $N$ be positive integers.  Assume $k$ is even.
\begin{problem} \label{problem: expansion}
Let $f \in S_k(\Gamma_0(N))$ be a normalized newform. Given the Fourier expansion of $f$ at the cusp $\infty$, computing its expansion at all other cusps of $X_0(N)$.
\end{problem}
Problem~\ref{problem: expansion} is equivalent to computing the map
\[
	S_k(\Gamma_0(N)) \times \SL_2(\bZ)  \to S_k(\Gamma(N)): (f, \alpha) \mapsto f | \alpha, 
\]
with respect to a basis of $S_k(\Gamma_0(N))$.
The case when $N$ is square-free was solved by Asai in \cite{asai1976fourier}.  For general $N$, Delaunay computed some examples in \cite{delaunay2002thesis}. In \cite{edixhoven2006computational}, Bosman envisioned a numerical algorithm. \\

The expansions at all cusps are useful  in evaluating modular functions on $X_0(N)$ near the cusps, which is an important part of the algorithms of  \cite{edixhoven2006computational} to compute the residual Galois representations $\bar{\rho}_{f,\lambda}$ associated to the newform $f$.   \\

Also, if $f$ is the newform attached to an elliptic curve over $\bQ$, then the expansions of $f$ at all cusps contain information of the ramification divisor $R_\varphi$. More precisely, I proved the following theorem.
\begin{theorem}
If $f$ is the newform attached to an elliptic curve $E/\bQ$ of conductor $N$. Let $F = \prod_{\alpha \in \Gamma_0(N) \backslash \SL_2(\bZ)} f | \alpha$ and let $(d_1) = \{a \in \bZ: aF(q) \in \bZ[[q]]\}$ and $(d_2) = \{a \in \bZ: aj(z) \in \bar{\bZ} \mbox{ for all } z \in R_\varphi \}$. Then $d_1 = \pm d_2$.
\end{theorem}

I will derive a formula for the $q$-expansions of newforms at all cusps in terms the levels, $q$-expansions at $\infty$, and pseudo-eigenvalues under Atkin-Lehner involutions for a set of twists of $f$. Then I gave numerical algorithms to compute these quantities associated to twists of $f$, hence solving Problem~\ref{problem: expansion}. The code for the algorithms is at \url{https://github.com/haochenuw/qexpansion}. \\

My approach appears to be more efficient than the one proposed in \cite{edixhoven2006computational} in many cases. Their method requires working at level $NR^2$, where $R^2$ is the largest square divisor of $N$, and it involves computing the entire period matrix of $X_0(N)$ and $X_0(NR^2)$. In contrast, our algorithm works at levels dividing $N$, and performs a few period integral evaluations. Also, our algorithm is more specific, since in order to compute the expansions of a single newform,
we only need information on a small set of twists, so we do not need to go over the whole space of cusp forms of weight $k$ and level $N$. \\


\subsection{Chapter 5}
In Chapter 5, I study {\it Chow-Heegner points}. Let $E,F$ be non-isogenous elliptic curves defined over $\bQ$ of the same conductor $N$. The Chow-Heegner point $P_{E,F} \in E(\bQ)$ is constructed by the following procedure: take any point on $F(\bC)$, take its inverse image on $X_0(N)$, then map that image down to $E$ and take the sum the resulting points. In \cite{darmon2015algorithms}, Darmon, Daub, Lichtenstein and Rotger developed an algorithm to compute Chow-Heegner points via iterated integrals. In \cite{stein2012numerical}, Stein developed a fast and conceptually easy algorithm to numerically compute Chow-Heegner points. The following theorem is proved by Yuan-Zhang-Zhang in \cite{yuan2011triple}: 
...
\end{theorem}
When the rank of $E(\bQ)$ is one, we consider the index $i_{E,F} = [E(\bQ)/tors: \bZ P_{E,F}]$. Theorem~\ref{thm: yzz} combined with the Bloch-Kato conjecture on critical values of motivic $L$-functions suggests that this index might be linked to interesting arithmetic invariants related to $E$ and $F$.  \\
...
Also, combining the method developed in Chapter 3 with the PARI function {\bf ell.taniyama}, I can develop an algebraic algorithm to compute lifting of points of any modular parametrisation $X_0(N) \to E$.  Using this, I will give an algebraic algorithm to {\it provably} compute the Chow-Heegner point $P_{E,F}$.


\section{Future plans}

\subsection{critical subgroups}
A natural next step is to compute $E_{\crit}(\bQ)$ for the first elliptic curve of rank three.
\begin{project}
Compute $E_{\crit}(\bQ)$ for $E = {\bf 5077a}$.
\end{project} 
This has not been done, and there has not been any heuristics. For all we know, this critical subgroup might contain a non-torsion point. This computation is beyond the reach of the current implementation of my algorithm. I plan to improve my algorithm to compute it. 

\begin{project}
Prove or disprove that $\rank(E_{\crit}(\bQ)) = 0$ whenever $r_{\an}(E)$ is even.
\end{project} 
For example, we could consider factorization of critical polynomials and the action of Atkin-Lehner involutions. This strategy is suggested by the data I obtained of the factorization of critical polynomials. \\

Also, there exists parametrizations of certain elliptic curves by Shimura curves. This should give rise to analogs of critical subgroup, which I plan to investigate further.  

\subsection{expansions}
Using my algorithm of computing Fourier expansions, I can determine the epsilon  factors attached to twists of the automorphic representation $\pi_{f}$ of $\GL_2(\bA_\bQ)$ attached to the newform $f$, which in turn determines the representation $\pi_f$ itself. Loeffler and Weinstein developed an algebraic algorithm to compute the local components $\pi_{f,p}$ for $p \mid N$ using modular symbols in \cite{loeffler2010computation}. However, they did not give complexity bounds for their algorithm.

\begin{project}
Use my algorithm to develop a numerical algorithm for computing local components $\pi_{f,p}$, with explicit complexity bounds.
\end{project}

\begin{project}
Prove the ``prime divisors of first term'' observation in Section~\ref{sec: norm of first term}.
\end{project}

\subsection{Chow-Heegner points}
\begin{project}
Prove that the index of $P_{E,F}$ is even in all cases.
\end{project}
\begin{project}
Provably verify the data in the table of Chow-Heegner points in \cite{stein2012numerical} and extend the table.
\end{project}
...
In this chapter we give some background on elliptic curves, modular forms, and automorphic representations. 
All information in this chapter can be found in the book of Silverman \cite{silverman2009arithmetic}, the book on modular forms by Diamond and Shurman \cite{diamond2006first}, and the paper by Loeffler and 
Weinstein \cite{loeffler2010computation}.  Readers familiar with these materials can safely skip this chapter. We remark that the knowledge on automorphic representations is needed only for Section~\ref{sec: local components}. 

\section{Algebraic curves}
Let $K$ be a field.
A {\em nonsingular projective curve} $X/K$ is an smooth projective variety $X \subset \bP_K^n$ of dimension one. We'll abbreviate it with ``curve''.

For every curve $X$ there is a nonnegative integer $g(X)$ called its {\em genus}, and a field $k(X)$ of transcendence degree 1 over $K$, called the {\em field of rational functions on} $X$. We have the following fact:
\begin{Fact}
\label{fact: degree}
If $x$ is a non-constant rational function on $X$,  then $[K(X): K(x)] = \deg x$.
\end{Fact}

Let $\varphi: X \to Y$ be a finite morphism between two curves; then there exists a positive integer $d = \deg(\varphi)$ such that for all but finitely many points $y \in Y$, the set $\varphi^{-1}(y)$ consists of $d$ distinct points. 
\begin{Definition}
A point $x \in X$ is {\em ramified} under $\varphi$ if $|\varphi^{-1}(\varphi(x))|<d$.
\end{Definition}
Fix the morphism $\varphi$. For each $x \in X$ there is a positive integer $e_{\varphi}(x)$ called the {\em ramification index of $\varphi$ at $x$}, and we say that $x$ is {\em ramified under $\varphi$} if $e_{\varphi}(x) > 1$.

Let $\bar{k}$ be a fixed algebraic closure of $k$. A {\em divisor} on a curve $X$ is a formal sum $D = \sum_{z \in X(\bar{k})} n_z[z]$ where $n_z = 0$ for all but finitely many $z \in X$. The set of divisors on $X$ form a free abelian group, which is denoted by $\Div(X)$. 

The absolute Galois group $G_K := \Gal(\bar{K}/K)$ acts on the group $\Div(X)$ of divisors by $\sigma(\sum n_z[z]) = \sum n_z[\sigma(z)]$. We say a divisor $D \in \Div(X)$ is {\em defined over $K$} if it is invariant under this action. The subgroup of $G_K$-invariant divisors is denoted by $\Div_k(X)$. 
...
For any nonzero meromorphic differential $\omega$ on $X$,  we have 
\[
...
Let $K$ be a field. An {\em elliptic curve defined over $K$} is the projective closure in $\bP_K^2$ of the affine curve given by the Weierstrass equation $y^2 = x^3 + Ax+B$ with $A,B \in K$ and $4A^3+27B^2 \neq 0$. Alternatively, an elliptic curve over $K$ is a curve over $K$ of genus one with a distinguished $K$-rational point $\cO \in E(K)$. 

Let $E$ be an elliptic curve defined over $K$.  It turns out that $E$ also has the structure of an algebraic group. 
...
this group law is algebraic, commutative and associative. Therefore, an elliptic curve over $K$ is also an abelian variety over $K$ of dimension 1. In particular, for any extension $K'/K$, the set $E(K')$ has the structure of an abelian group, with the identity element being $\cO$. 
...
Let $E$ be an elliptic curve defined over $\bQ$. For each prime $p$, there exists a reduced curve $\tilde{E}$ defined over the finite field $\bF_p$, which is either an elliptic curve or a singular curve. If $\tilde{E}$ is an elliptic curve, then we say that $p$ is a prime of {\em good reduction}, otherwise, we say $p$ is of {\em bad reduction}. Suppose $p$ is a prime of good reduction, then we have a 
reduction map $\rho_p: E \to \tilde{E}$. Also, let $N_p$ denote the cardinality of the finite group $\tilde{E}(\bF_p)$, and let 
\[
	a_p = p+1 - N_p. 
\]
...
Let $E/\bQ$ be an elliptic curve. Then there is a positive integer $N$, called the {\em conductor of $E$}, with the property that $N$ is divisible exactly by the primes of bad reduction. When $p$ is a prime of bad reduction, there is a procedure to determine the highest power of $p$ dividing $N$, which we omit here. We only mention that when $p \geq 5$, we have $\ord_p(N) \leq 2$; when $p = 3, \ord_p(N) \leq 5$; when $p = 2, \ord_p(N) \leq 8$. 
...
-1 & \mbox{ non-split multiplicative reduction } \\
0 & \mbox{ otherwise }
\end{cases} 
\]
The {\em local $L$-factor of $E$ at $p$} is $L_p(E,T) = 1 - a_pT + pT^2$ if $p$ is of good reduction and $1 - a_p T$ otherwise.
Finally, the {\em $L$-function attached to $E$} is defined as the following Euler product 
\[
...
Let $\H = \{z \in \bC: \Im(z) > 0\}$ be complex upper half plane.
...
Recall that $\H = \{z \in \bC: \Im(z) > 0\}$ is the complex upper half plane. For each integer $k$, the weight-$k$ action of $SL_2(\bZ)$ on functions $f: H \to \bC$ is defined as follows: suppose $g  = \abcd{a}{b}{c}{d} \in GL_2(\bQ)$, define
\[
...
\end{Definition}
A modular form $f$ is a {\em cusp form} if $f$ vanishes at all cusps. For the precise definition of vanishing at cusps, see  \cite[I.2]{diamond2006first}. By definition, if $f$ is a modular form, then f is 1-periodic: $f(z+1) = f(z)$. It has a $q$-expansion 
\[
...
We are going to be interested in a subset of cusp forms on $\Gamma_0(N)$ called  {\em newforms}In particular, all cusp forms attached to elliptic curves defined over $\bQ$ are newforms. 

For each positive integer $N$, we have an elementary abelian 2-group $W \subseteq Aut_\bQ(X_0(N))$, which we 
call the {\em Atkin-Lehner Group}, with generators $\{w_p\}_{p \mid N}$. The non-trivial elements of $W$ are called {\em Atkin-Lehner} involutions. They act on $S_k(\Gamma_0(N))$ by invertible linear transformations. Any newform $f$ is an eigenvector of every $w \in W$ with eigenvalue $\pm1$, i.e., $f|w = \pm f$. In particular, if $E/\bQ$ is an elliptic curve and $f$ is the modular form attached to $E$, then we have $f|{w_N} = (-1)^{r_{an}(E)}f$ where $w_N = \prod_{p \mid N} w_p$ is also called the Fricke involution, and $r_{an}(E)$ is the analytic rank of $E$ 
we will define later. 


\subsection{Hecke operators}

Let $p$ be a prime, the Hecke operator $T_p$ acts on $S_k(\Gamma_1(N))$ as follows. Write the double coset 
$\Gamma_1(N) \abcd{1}{0}{0}{p} \Gamma_1(N)$ as $\sum_{i=1}^n \Gamma_1(N) \beta_i$.  Then we define 
\[
	T_p f  =   \sum_{i=1}^n f |[\beta_i]_k.
\]

For $d \in (\bZ/N\bZ)^*$,  the diamond operators $\langle d \rangle$ is defined as follows. Choose any matrix $\gamma_d \in \Gamma_0(N)$ with lower entry $d$ and 
\[
	\langle d \rangle f :=  f | [\gamma_d]_k.
\]	
We have the following decomposition 
$$S_k(\Gamma_1(N) = \oplus_{\chi : (\bZ/N\bZ)^* \to \bC^*} S_k(\Gamma_1(N),\chi)$$
where $S_k(\Gamma_1(N),\chi)$ is the subspace of $S_k(\Gamma_1(N))$ on which the diamond operators acts via 
the character $\chi$. In particular, we have $S_k(\Gamma_0(N)) = S_k(\Gamma_1(N), \chi_0)$.  Note that the $T_p$ operators commute with the diamond operators. Hence $S_k(\Gamma_0(N))$ is stable under the $T_p$.  
...
\subsection{Newforms}
The Petersson inner product on two cusp forms on some congruence group $\Gamma$ is 
\[
\langle f, g \rangle = \frac{1}{V_\Gamma}  \int_{X(\Gamma)} f(z)\bar{g(z)} \Im(z)^k d\mu(z)
\]
where $\mu$ is the hyperbolic measure on the upper half plane.

Suppose we have two levels $M \mid N$. Then for any divisor $d$ of $N/M$, there is a degeneracy map 
\[
	\alpha_d: S_k(M) \to S_k(N): f \mapsto f|[\abcd{d}{0}{0}{1}]_k.
\]

\begin{Definition}
The old subspace of $S_k(N)$ is the span of $\alpha_d(S_k(M))$ where $M, d$ range over all divisors of $N$ s.t. $Md \mid N$. The new subspace of $S_k(N)$ is the orthogonal complement of the old subspace under the Petersson inner product. 
\end{Definition}

The new subspace of $S_k(N)$ has a basis consisting of simultaneous eigenforms under $T_p$ for all primes $p$. 
Given such an eigenform $f  = \sum a_n q^n$, we could scale it so that $a_1 = 1$. The result is called a {\em newform}.
...
$\bA_\bQ$ be the group of the adeles over $\bQ$. Then $f$ corresponds to an automorphic representation of 
...
\begin{theorem} \label{thm: direct method}
If
\[
...
The following lemma is a slight generalization of Lemma \ref{lemma: ono}. 
\begin{Lemma}
...
\begin{algorithmic} [1]               % enter the algorithmic environment
    ...
    \State{Use Lemma~\ref{lemma: norm-sqfree} to compute the $q$-expansion of $\Norm_N(f)$.}
    \State{Use Theorem~\ref{thm: direct method} to compute $F_f(q)$. Normalize $F_f(q) $ so that it has leading coefficient 1}.
...
If the answer to Question~\ref{q1} is positive, then we would know $E_{\crit}(\bQ)$ is torsion whenever $r_{an}(E) \geq 2$. 
...
Section~\ref{sec: examples} contains some numerical examples. In Section 10 and 11, we investigate the factorizations of norm of the first term $a_1(f_{z})$, which is of independent interest. % In section, we describe an alternative approach and 
...
Let $E = {\bf 50a}$ and consider the four cusps of denominator 10 on $X_0(50)$. The corresponding first terms 
...
\[
	f_{[1/10]}(q) = \sum_{n \geq 1} a_n(f) a_1(f, \frac{n'}{10}) q^n.
\]
where for each $n \geq 1$, we take $n'$ to be the unique element in $(\bZ/10\bZ)^{\times}$ such that $n' \equiv n \mod{5}$. 
\begin{Example}
Let $E = {\bf 48a}$ and let $c = [1/12]$. We computed numerically that 
...
Since the coefficient of $q$ in $f_c(q)$ vanishes, we conclude that the modular parametrization $\varphi: X_0(48) \to A_f$ is ramified at the cusp $c$. 
\end{Example}

\begin{Example}
Let $E = {\bf 98a}$ and $c = [\frac{1}{14}]$. In this example (as well as the above one), the modular form $f$ attached to $E$ is not twist-minimal. More precisely, if $\chi$ is the quadratic character modulo 7, then 
\[
...
\section{Automorphic representations; norm of first terms} \label{sec: local components}
...
In this section, we restrict ourselves to the case when the Fourier coefficients of $f$ are rational numbers. Then 
$f$ induces an admissible representation $\pi_f$ of $GL_2(\bA_\bQ)$. We will see that the expansion of $f$ at all cusps can also be computed from the local component $\pi_{f,p}$. Loeffler and Weinstein gave an algorithm to compute such 
local components. 
...
\]
an algebraic number with minimal polynomial $x^{20} + 109/121 x^15 + 2861/1331 x^10 + 109/121x^5 + 1$.
...
\section{Norm of first terms computations and an interesting observation} \label{sec: norm of first term}
...
takes an especially simple form.  We summarize these observations in the lemma below. 
\begin{Lemma}
...
One motivation to study the factorization of $a_1(f_{z_p})$ as a principal fractional ideal in $\bQ(\zeta_p)$ is that they relate to critical points of modular parameterization of $E$ in the following way:
...
\begin{proof}
By Lemma~\ref{newton polygon}, the condition implies that $f_j$ converges on $D(1)$ for all $j$ and $f_1$ has a root in $D(1)$. Since $F$ is the product of the $f_j$'s, we know that $F$ has a root in $D(1)$. Hence the claim follows, again from Lemma~\ref{newton polygon}. 
\end{proof}

Fix a prime $\fp$ above $p$ in $\bar{\bQ}$. We say a Laurent series $f \in \bar{\bQ}((q))$ is {\em integral at $\fp$} if $f = q^s(1 + \sum_{n \geq 1} a_n q^n)$ with $\ord_{\fp}(a_n) \geq 0$ for all $n$. One sees that if $f$ is integral at $\fp$, then $\frac{1}{f}$ is also integral at $\fp$. Product of two integral power series is integral. We similarly define the notion for a monic polynomial $F[x] \in \bQ[x]$ to be integral at $p$. 
...
Without loss of generality, we may assume  $s_1 = (135 \cdots g-1)$. Then $I = (\fl_1 \fl_3 \ldots \fl_{g-1})^t$ for some positive integer $t$. Hence 
...
\begin{algorithmic} [1]               % enter the algorithmic environment
    \Require  $E, F$  = non-isogeneous elliptic curves of conductor $N$; $q$-expansions of $x_E, y_E$, $x_F,y_F$. 
    \Ensure the Chow-Heegner point $P_{E,F}$. 
    \State{$u_E \Leftarrow (x_F)^{-1}$ and $u_F \Leftarrow (x_F)^{-1}$}. 
    \State{Mimicking steps 4-7 of Algorithm~\ref{IPR},  compute an irreducible polynomial $F(x,y)$ such that $F_{E,F}(u_E,u_F) = 0$}. 
