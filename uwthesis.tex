...
The main result of this chapter is Theorem~\ref{thm: ExpansionFormula} and Algorithm~\ref{alg: qexp}, which computes the Fourier expansion of a newform $f \in S_k(\Gamma_0(N))$ at any width one cusp $z$. Along the way, we have developed algorithms to compute the twists $f \otimes \chi$ and the pseudo-eigenvalue of newforms under the Fricke involution. Section contains some examples. In section, we describe an alternative approach and 
...
We found that $f \otimes \chi$ is a newform of level 14, with $q$-expansion (fixme: add expansion).
\end{Example}
